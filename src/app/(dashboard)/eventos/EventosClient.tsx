      ...
      <div style={{ background: C.bg1, border: `1px solid ${isPast ? C.border : accent + '33'}`, borderRadius: 20, padding: 20, opacity: isPast ? 0.7 : 1 }}>
        <div style={{ display: 'flex', gap: 14 }}>
          ...
          <div style={{ width: 56, height: 56, borderRadius: 14, background: accent + '15', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: `1px solid ${accent}33` }}>
            <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 20, color: accent, lineHeight: 1 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <h3 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 15, color: C.text, marginBottom: 4 }}>{event.title}</h3>
        ...
        {!isPast && (
          <div style={{ display: 'flex', gap: 10, marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
            <button onClick={() => toggleRsvp(event.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 10, background: hasRsvp ? 'rgba(0,214,143,0.1)' : `linear-gradient(135deg, ${accent}, ${accent}cc)`, color: hasRsvp ? C.green : '#fff', border: hasRsvp ? `1px solid rgba(0,214,143,0.3)` : 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 13 }}>
  ...
  return (
    <div style={{ padding: 28, maxWidth: 800, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 30, letterSpacing: '-0.04em', color: C.text, marginBottom: 4 }}>Calendario 📅</h1>
          <p style={{ fontSize: 13, color: C.muted }}>{upcoming.length} eventos próximos</p>
        </div>
        {isCreator && ownedCommunities.length > 0 && (
          <button onClick={() => setShowCreate(!showCreate)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', borderRadius: 12, background: showCreate ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 13 }}>
      ...
      {showCreate && (
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, padding: 24, marginBottom: 24 }}>
          <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 16, color: C.text, marginBottom: 20 }}>Nuevo evento</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button onClick={createEvent} disabled={saving} style={{ padding: '13px', borderRadius: 12, background: 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', border: 'none', cursor: saving ? 'not-allowed' : 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 14, opacity: saving ? 0.7 : 1 }}>
      ...
      {upcoming.length > 0 ? (
        <div style={{ marginBottom: 32 }}>
          <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 14, color: C.muted, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 14 }}>Próximos</h2>
        ...
        <div style={{ textAlign: 'center', padding: '60px 24px', background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, marginBottom: 24 }}>
          ...
          <h3 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 18, color: C.text, marginBottom: 8 }}>Sin eventos próximos</h3>
      ...
      {past.length > 0 && (
        <div>
          <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 14, color: C.muted, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 14 }}>Pasados</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
