  ...
  return (
    <div style={{ padding: '32px 28px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 'clamp(28px,5vw,42px)', letterSpacing: '-0.04em', lineHeight: 1, marginBottom: 8 }}>
          🎮 Juegos
        </h1>
      ...
      <div style={{ display: 'flex', gap: 20, marginBottom: 28, alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, var(--purple), var(--purple2))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, fontFamily: 'Syne, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 3, marginBottom: 24, background: 'var(--bg1)', borderRadius: 12, padding: 3, width: 'fit-content', border: '1px solid var(--border)' }}>
        {([['spin', '🎡 Spin Diario'], ['trivia', '🧠 Trivia']] as const).map(([g, label]) => (
          <button key={g} onClick={() => setActiveGame(g)}
            style={{ padding: '8px 20px', borderRadius: 10, fontSize: 13, fontWeight: 700, fontFamily: 'Syne, sans-serif', cursor: 'pointer', border: 'none', background: activeGame === g ? 'var(--bg3)' : 'transparent', color: activeGame === g ? 'var(--text)' : 'var(--muted2)', transition: 'all .15s' }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 24, alignItems: 'start' }}>
                  ...
                  <div>
                    {spinResult ? (
                      <div style={{ background: `${PRIZES[0].color}15`, border: `1px solid ${spinResult.color}33`, borderRadius: 12, padding: '14px 20px', display: 'inline-block' }}>
                        ...
                        <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 16, color: spinResult.color }}>{spinResult.label}</div>
          ...
          <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 16, padding: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14 }}>
              ...
              <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 13 }}>Jackpots esta semana</span>
            </div>
            {topWinners.length === 0 ? (
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < topWinners.length - 1 ? '1px solid var(--border)' : 'none' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg,var(--gold),var(--gold2))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, fontFamily: 'Syne, sans-serif', color: '#0A0A12', flexShrink: 0 }}>
        ...
        <div style={{ maxWidth: 640 }}>
          ...
          <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 20, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '24px 22px' }}>
              {/* Question */}
              <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 18, lineHeight: 1.3, marginBottom: 22 }}>
                {trivia.q}
              </h2>
                ...
                {trivia.opts.map((opt, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => answerTrivia(i)} disabled={triviaAnswered}
                      style={{ width: '100%', padding: '13px 16px', borderRadius: 12, background: bg, border: `1px solid ${border}`, color, fontSize: 14, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 500, cursor: triviaAnswered ? 'default' : 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 10, transition: 'all .15s' }}>
                      <span style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, fontFamily: 'Syne, sans-serif', flexShrink: 0 }}>
                        {['A', 'B', 'C', 'D'][i]}
                      </span>
