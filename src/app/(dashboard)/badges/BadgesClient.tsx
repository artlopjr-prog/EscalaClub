  ...
  return (
    <div style={{ padding: '32px 28px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 'clamp(28px,5vw,42px)', letterSpacing: '-0.04em', marginBottom: 6 }}>
          🎖 Mis Badges
        </h1>
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(123,94,248,0.1), rgba(233,160,32,0.05))', border: '1px solid rgba(123,94,248,0.2)', borderRadius: 20, padding: '24px 28px', marginBottom: 28, display: 'flex', gap: 28, alignItems: 'center' }}>
        <div style={{ textAlign: 'center', flexShrink: 0 }}>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 52, lineHeight: 1, color: 'var(--purple2)' }}>{earnedCount}</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 3, background: 'var(--bg1)', borderRadius: 10, padding: 3, border: '1px solid var(--border)' }}>
          {[['all','Todos','🏅'], ['earned','Obtenidos','✅'], ...Object.entries(CAT_CONFIG).map(([k,v]) => [k, v.label, v.emoji])].map(([k, label, emoji]) => (
            <button key={k} onClick={() => setFilter(k)}
              style={{ padding: '6px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', background: filter === k ? 'var(--bg3)' : 'transparent', color: filter === k ? 'var(--text)' : 'var(--muted2)', transition: 'all .15s', whiteSpace: 'nowrap' }}>
              {emoji} {label}
            </button>
          ))}
        </div>
        ...
        {/* Rarity */}
        <div style={{ display: 'flex', gap: 3, background: 'var(--bg1)', borderRadius: 10, padding: 3, border: '1px solid var(--border)' }}>
          <button onClick={() => setRarityFilter('all')} style={{ padding: '6px 10px', borderRadius: 8, fontSize: 11, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', background: rarityFilter === 'all' ? 'var(--bg3)' : 'transparent', color: rarityFilter === 'all' ? 'var(--text)' : 'var(--muted)', transition: 'all .15s' }}>Todos</button>
          {Object.entries(RARITY_CONFIG).map(([rarity, cfg]) => (
            <button key={rarity} onClick={() => setRarityFilter(rarity)}
              style={{ padding: '6px 10px', borderRadius: 8, fontSize: 11, fontWeight: 700, border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', background: rarityFilter === rarity ? cfg.border : 'transparent', color: rarityFilter === rarity ? cfg.color : 'var(--muted)', transition: 'all .15s' }}>
              {cfg.label}
            </button>
          ))}
        </div>
        ...
        {filtered.map(b => {
          ...
          return (
            <div key={b.id} onClick={() => setSelected(b === selected ? null : b)}
              ...
              {/* Name */}
              <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 12, color: b.earned ? 'var(--text)' : 'var(--muted)', marginBottom: 4, lineHeight: 1.2 }}>
                {b.name}
              </div>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 16 }}>{selected.name}</div>
