  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      <div style={{ background: C.bg1, borderBottom: `1px solid ${C.border}`, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 16, color: C.text, flex: 1 }}>❓ Q&A — {community.name}</h1>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, padding: 18, marginBottom: 24 }}>
          ...
          <textarea value={newQ} onChange={e => setNewQ(e.target.value)} placeholder="¿Qué quieres preguntarle al creador?" rows={3} style={{ width: '100%', background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: '12px 14px', color: C.text, fontSize: 14, outline: 'none', resize: 'none', fontFamily: 'Plus Jakarta Sans, sans-serif' }} />
        ...
        {questions.length > 0 ? questions.map(q => (
          ...
          <div style={{ textAlign: 'center', padding: '60px 24px', background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20 }}>
            ...
            <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 18, color: C.text, marginBottom: 8 }}>Sin preguntas aún</h3>
