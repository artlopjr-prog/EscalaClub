...
const C = { bg1: '#0D0D14', border: 'rgba(255,255,255,0.07)', text: '#EEEDF5', muted: '#6B6A80', muted2: '#9998B0', red: '#FF4D6A' }
...
export default function LeaveButton({ communityId, communityName, membershipId, userId, isPaid, accessUntil }: Props) {
  ...
  return (
    <>
      <button onClick={() => setShowModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 16px', borderRadius: 12, background: 'rgba(255,77,106,0.08)', border: '1px solid rgba(255,77,106,0.2)', color: C.red, cursor: 'pointer', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 13 }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 24, padding: 32, width: '100%', maxWidth: 420, position: 'relative' }}>
            ...
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 20, color: C.text, marginBottom: 10 }}>¿Salir de {communityName}?</h2>
            {isPaid ? (
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleLeave} disabled={loading} style={{ flex: 1, padding: '12px', borderRadius: 12, background: 'linear-gradient(135deg, #FF4D6A, #cc3355)', border: 'none', color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, opacity: loading ? 0.7 : 1 }}>
