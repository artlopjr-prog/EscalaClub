...
export default function JoinButton({ communityId, communityName, communitySlug, accessType, priceMonthly, paypalEmail, isMember, accentColor, userId }: Props) {
  ...
  // Already a member
  if (joined) return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 20px', borderRadius: 12, background: 'rgba(0,214,143,0.1)', border: '1px solid rgba(0,214,143,0.3)', color: C.green, fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 24, padding: 32, width: '100%', maxWidth: 420, position: 'relative' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 20, color: C.text, marginBottom: 6 }}>Unirse a {communityName}</h2>
              <p style={{ fontSize: 14, color: C.muted, marginBottom: 4 }}>Membresía mensual</p>
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 36, color: C.text }}>${priceMonthly}<span style={{ fontSize: 16, color: C.muted, fontWeight: 400 }}>/mes</span></div>
            </div>
