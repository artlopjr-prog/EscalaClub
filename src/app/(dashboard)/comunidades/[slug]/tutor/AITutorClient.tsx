        ...
        <div style={{ width: 256, flexShrink: 0, borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', background: 'var(--bg1)' }}>
          ...
          <div style={{ padding: '20px 16px 12px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, fontFamily: 'Syne, sans-serif' }}>{community.name}</div>
            ...
            <button onClick={startNewConversation}
              style={{ width: '100%', padding: '9px 12px', borderRadius: 'var(--r-md)', background: `${accent}18`, border: `1px solid ${accent}33`, color: accent, fontSize: 12, fontWeight: 700, fontFamily: 'Syne, sans-serif', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, transition: 'all .15s' }}>
          ...
          {isOwner && (
            <div style={{ padding: '10px 8px', borderTop: '1px solid var(--border)' }}>
              <button onClick={() => setConfigMode(!configMode)}
                style={{ width: '100%', padding: '8px 12px', borderRadius: 'var(--r-sm)', background: 'transparent', border: 'none', color: 'var(--muted2)', fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
            <div style={{ maxWidth: 600, margin: '0 auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
                ...
                <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 20 }}>Configurar Tutor IA</h2>
              </div>
              ...
              <div style={{ maxWidth: 720, margin: '0 auto' }}>
                ...
                {isEmpty && (
                  <div style={{ textAlign: 'center', padding: '48px 0 32px' }}>
                    ...
                    <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 22, marginBottom: 8 }}>
                      {tutorName}
                    </h2>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, maxWidth: 500, margin: '0 auto' }}>
                      {SUGGESTED.map((s, i) => (
                        <button key={i} onClick={() => sendMessage(s)}
                          style={{ padding: '10px 14px', borderRadius: 'var(--r-md)', background: 'var(--bg2)', border: '1px solid var(--border2)', color: 'var(--text2)', fontSize: 12, cursor: 'pointer', textAlign: 'left', fontFamily: 'Plus Jakarta Sans, sans-serif', transition: 'all .15s', lineHeight: 1.4 }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  {messages.map(m => (
                    <div key={m.id} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexDirection: m.role === 'user' ? 'row-reverse' : 'row' }}>
                      {/* Avatar */}
                      <div style={{ width: 32, height: 32, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, fontFamily: 'Syne, sans-serif', background: m.role === 'assistant' ? `${accent}22` : 'var(--bg3)', border: m.role === 'assistant' ? `1px solid ${accent}33` : '1px solid var(--border2)', color: m.role === 'assistant' ? accent : 'var(--text2)' }}>
              ...
              <div style={{ maxWidth: 720, margin: '0 auto' }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', background: 'var(--bg2)', border: `1px solid var(--border2)`, borderRadius: 'var(--r-lg)', padding: '10px 14px', transition: 'border-color .15s, box-shadow .15s', boxShadow: 'none' }}
                  ...
                  <textarea ref={inputRef} value={input}
                    ...
                    disabled={loading}
                    rows={1}
                    style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: 'var(--text)', fontSize: 14, fontFamily: 'Plus Jakarta Sans, sans-serif', resize: 'none', lineHeight: 1.5, maxHeight: 120, overflow: 'auto', padding: 0 }} />
