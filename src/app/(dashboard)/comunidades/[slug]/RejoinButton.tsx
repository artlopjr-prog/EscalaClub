...
const C = { bg1: 'var(--bg1)', border: 'var(--border)', text: 'var(--text)', muted: 'var(--muted)', gold: '#F0A500' }
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 24, padding: 32, width: '100%', maxWidth: 420, position: 'relative' }}>
            ...
            <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--muted2)', marginBottom: 8, fontFamily: 'Inter, sans-serif', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Tu mensaje (opcional)</label>
            <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Ej: Creo que fue un malentendido, me comprometo a respetar las reglas..." style={{ width: '100%', background: 'var(--bg1)', border: `1px solid ${C.border}`, borderRadius: 12, padding: '12px 14px', color: C.text, fontSize: 13, outline: 'none', resize: 'vertical', minHeight: 80, boxSizing: 'border-box', fontFamily: 'Inter, sans-serif' }} />
