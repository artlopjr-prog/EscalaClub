  ...
  return (
    <div style={{ padding: 28, maxWidth: 720, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        ...
        <div>
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 24, letterSpacing: '-0.04em', color: C.text, marginBottom: 2 }}>📢 Anuncios</h1>
      ...
      <div style={{ background: C.bg1, border: `1px solid rgba(240,165,0,0.2)`, borderRadius: 20, padding: 22, marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.gold }}>Nuevo anuncio</span>
        </div>
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Título del anuncio (opcional)" style={{ width: '100%', background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: '11px 14px', color: C.text, fontSize: 14, outline: 'none', marginBottom: 10, fontFamily: 'Syne, sans-serif', fontWeight: 700 }} />
        <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Escribe tu anuncio aquí. Todos los miembros recibirán una notificación..." rows={4} style={{ width: '100%', background: C.bg2, border: `1px solid ${C.border}`, borderRadius: 12, padding: '12px 14px', color: C.text, fontSize: 14, outline: 'none', resize: 'vertical', fontFamily: 'Plus Jakarta Sans, sans-serif', lineHeight: 1.6 }} />
      ...
      {/* Past announcements */}
      <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.muted, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 14 }}>Anuncios anteriores</h2>
      {announcements.length > 0 ? announcements.map((a, i) => (
        <div key={a.id} style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, marginBottom: 10 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ flex: 1 }}>
              {a.title && <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, color: C.text, marginBottom: 4 }}>{a.title}</div>}
