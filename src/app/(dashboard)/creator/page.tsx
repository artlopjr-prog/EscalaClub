  ...
  if (!community) {
    return (
      <div style={{ padding: 32, maxWidth: 600, margin: '80px auto', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 28, color: C.text, marginBottom: 12 }}>Crea tu comunidad</h1>
        <p style={{ fontSize: 15, color: C.muted, marginBottom: 28, lineHeight: 1.6 }}>Empieza a construir tu audiencia en EscalaClub.</p>
        <Link href="/creator/comunidad" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '14px 28px', borderRadius: 14, background: 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', textDecoration: 'none', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 15 }}>
          Crear mi comunidad →
        </Link>
      </div>
    )
    ...
    <div style={{ padding: 28, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 24, letterSpacing: '-0.04em', color: C.text, marginBottom: 3 }}>{community.name}</h1>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <Link href={`/comunidades/${community.slug}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 10, background: `linear-gradient(135deg, ${accent}, ${accent}cc)`, color: '#fff', textDecoration: 'none', fontSize: 13, fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14, marginBottom: 24 }}>
        {stats.map((s, i) => (
          <div key={i} style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 18, padding: 20 }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 34, letterSpacing: '-0.04em', color: s.color, lineHeight: 1 }}>{s.value.toLocaleString()}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 18, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, padding: 20 }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.text, marginBottom: 14 }}>Acciones rápidas</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
              {[
                ...
                <Link key={i} href={a.href} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '16px 12px', borderRadius: 14, background: C.bg2, border: `1px solid ${C.border}`, transition: 'all 0.15s' }}>
                  ...
                  <span style={{ fontSize: 11, fontWeight: 700, color: a.color, textAlign: 'center', fontFamily: 'Syne, sans-serif' }}>{a.label}</span>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: `1px solid ${C.border}` }}>
              <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.text }}>Actividad reciente</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: `1px solid ${C.border}` }}>
              <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 13, color: C.text }}>Miembros recientes</h3>
            ...
            {recentMembers && recentMembers.length > 0 ? recentMembers.map((m: any, i) => (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 18px', borderBottom: i < recentMembers.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: accent + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 12, color: accent, flexShrink: 0, overflow: 'hidden' }}>
