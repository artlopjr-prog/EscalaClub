...
export default function OnboardingPage() {
  ...
  const btn = (enabled: boolean, onClick: () => void, label: string) => (
    <button onClick={onClick} disabled={!enabled} style={{
      ...
      background: enabled ? 'linear-gradient(135deg, #7C3AED, #9F67FF)' : 'var(--border)',
      color: enabled ? '#fff' : 'var(--muted)', border: 'none',
      ...
      <div style={{ width: '100%', maxWidth: 560 }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: 'var(--muted)' }}>Paso {step} de 3</span>
            <span style={{ fontSize: 12, color: 'var(--muted)' }}>{Math.round((step/3)*100)}%</span>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 160px), 1fr))', gap: 14, marginBottom: 28 }}>
              {[
                ...
                <button key={opt.id} onClick={() => setRole(opt.id as 'member'|'creator')} style={{
                  ...
                  background: role === opt.id ? opt.color + '15' : 'var(--bg1)',
                  cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s',
                }}>
                  <div style={{ fontSize: 32, marginBottom: 12 }}>{opt.emoji}</div>
                  <h3 style={{ fontFamily: 'Inter, sans-serif', fontWeight: 800, fontSize: 16, marginBottom: 8, color: 'var(--text)' }}>{opt.title}</h3>
        ...
        {step === 2 && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--muted)' }}>Selecciona todos los que apliquen</p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 160px), 1fr))', gap: 10, marginBottom: 28 }}>
              {(role === 'creator' ? GOALS_CREATOR : GOALS_MEMBER).map(g => (
                <button key={g.id} onClick={() => toggleGoal(g.id)} style={{
                  ...
                  background: goals.includes(g.id) ? 'rgba(124,58,237,0.12)' : 'var(--bg1)',
                  cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s',
                }}>
                  <span style={{ fontSize: 22 }}>{g.icon}</span>
                  <span style={{ fontSize: 13, fontWeight: 500, color: goals.includes(g.id) ? 'var(--text)' : 'var(--muted2)', flex: 1 }}>{g.label}</span>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setStep(1)} style={{ padding: '12px 20px', borderRadius: 12, border: '1px solid var(--border2)', background: 'transparent', color: 'var(--muted)', cursor: 'pointer', fontSize: 14 }}>
                Atrás
              </button>
        ...
        {step === 3 && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--muted)' }}>Selecciona al menos 2</p>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
              {TOPICS.map(t => (
                <button key={t} onClick={() => toggleTopic(t)} style={{
                  ...
                  background: topics.includes(t) ? '#7C3AED' : 'var(--bg1)',
                  color: topics.includes(t) ? '#fff' : 'var(--muted2)',
              ...
              <button onClick={() => setShowOther(!showOther)} style={{
                padding: '9px 18px', borderRadius: 99, fontSize: 13, fontWeight: 500,
                background: showOther ? '#7C3AED' : 'var(--bg1)',
                color: showOther ? '#fff' : 'var(--muted2)',
            ...
            <div style={{ marginBottom: 24, fontSize: 12, color: totalTopics >= 2 ? '#00D68F' : 'var(--muted)' }}>
              {totalTopics} de mínimo 2 seleccionados
            </div>

            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setStep(2)} style={{ padding: '12px 20px', borderRadius: 12, border: '1px solid var(--border2)', background: 'transparent', color: 'var(--muted)', cursor: 'pointer', fontSize: 14 }}>
                Atrás
              </button>
              <button
                ...
                style={{
                  flex: 1, padding: '14px', borderRadius: 12,
                  background: totalTopics >= 2 ? 'linear-gradient(135deg, #7C3AED, #9F67FF)' : 'var(--border)',
                  color: totalTopics >= 2 ? '#fff' : 'var(--muted)', border: 'none',
