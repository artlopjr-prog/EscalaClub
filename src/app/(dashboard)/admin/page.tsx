  ...
  return (
    <div style={{ padding: 32, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
        ...
        <div>
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 28, letterSpacing: '-0.04em', color: C.text, marginBottom: 2 }}>Admin Panel</h1>
          <p style={{ fontSize: 13, color: C.muted }}>Vista general de EscalaClub</p>
        </div>
        <div style={{ marginLeft: 'auto', padding: '4px 12px', borderRadius: 99, background: 'rgba(124,58,237,0.15)', color: C.purple2, fontSize: 11, fontFamily: 'Syne, sans-serif', fontWeight: 700 }}>
          SUPER ADMIN
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginBottom: 32 }}>
        {[
          ...
          <div key={i} style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20 }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 30, letterSpacing: '-0.04em', color: s.color, lineHeight: 1 }}>{s.value}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', borderBottom: `1px solid ${C.border}` }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.text }}>Usuarios recientes</h2>
          ...
          <div>
            {recentUsers?.map((u, i) => (
              <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 22px', borderBottom: i < (recentUsers.length - 1) ? `1px solid ${C.border}` : 'none' }}>
                <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(124,58,237,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 12, color: C.purple2, flexShrink: 0 }}>
        ...
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
          <div style={{ padding: '18px 22px', borderBottom: `1px solid ${C.border}` }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.text }}>Comunidades recientes</h2>
          </div>
          <div>
      ...
      {subscriptions && subscriptions.length > 0 && (
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
          <div style={{ padding: '18px 22px', borderBottom: `1px solid ${C.border}` }}>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, color: C.text }}>Suscripciones de creadores</h2>
          </div>
          <div>
            {subscriptions.map((s, i) => (
              <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 100px', gap: 16, alignItems: 'center', padding: '12px 22px', borderBottom: i < subscriptions.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                ...
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, color: C.green }}>${s.amount}/mes</div>
