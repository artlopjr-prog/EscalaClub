...
export default async function CertificadoPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  if (!isCompleted) {
    return (
      <div style={{ padding: 32, maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
        ...
        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 24, color: C.text, marginBottom: 12 }}>Curso no completado</h2>
        <p style={{ fontSize: 15, color: C.muted, marginBottom: 24 }}>Completa todas las lecciones para obtener tu certificado.</p>
        <Link href={`/cursos/${courseId}`} style={{ display: 'inline-flex', padding: '12px 24px', borderRadius: 12, background: 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', textDecoration: 'none', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 14 }}>
          Continuar curso
        </Link>
      </div>
    )
  ...
  return (
    <div style={{ padding: 32, maxWidth: 800, margin: '0 auto' }}>
      <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 28, letterSpacing: '-0.04em', color: C.text, marginBottom: 28 }}>🎓 Tu certificado</h1>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0D0D14, #1A1A26)', border: '2px solid rgba(240,165,0,0.3)', borderRadius: 24, padding: '48px 40px', textAlign: 'center', marginBottom: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ fontSize: 12, letterSpacing: '0.2em', color: C.gold, fontFamily: 'Outfit, sans-serif', fontWeight: 700, textTransform: 'uppercase', marginBottom: 20 }}>Certificado de Completación</div>
        <div style={{ fontSize: 16, color: C.muted, marginBottom: 8 }}>Esto certifica que</div>
        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 36, letterSpacing: '-0.04em', color: C.text, marginBottom: 8 }}>{profile?.display_name ?? 'Estudiante'}</h2>
        <div style={{ fontSize: 16, color: C.muted, marginBottom: 8 }}>ha completado exitosamente el curso</div>
        <h3 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 24, color: C.gold, marginBottom: 24, letterSpacing: '-0.03em' }}>{course?.title}</h3>
      ...
      <div style={{ textAlign: 'center' }}>
        <button style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', borderRadius: 12, background: 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', border: 'none', cursor: 'pointer', fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 14 }}>
