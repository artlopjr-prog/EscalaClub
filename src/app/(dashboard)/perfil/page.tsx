...
const INP: React.CSSProperties = { width: '100%', background: '#13131C', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, padding: '12px 16px', color: '#EEEDF5', fontSize: 14, outline: 'none', fontFamily: 'Plus Jakarta Sans, sans-serif' }
const L: React.CSSProperties = { display: 'block', fontSize: 11, fontWeight: 700, color: '#6B6A80', marginBottom: 8, fontFamily: 'Syne, sans-serif', letterSpacing: '0.06em', textTransform: 'uppercase' }
    ...
    <div style={{ padding: 28, maxWidth: 860, margin: '0 auto' }}>
      ...
      <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 24, overflow: 'hidden', marginBottom: 20 }}>
        ...
        <div style={{ padding: '0 24px 24px', marginTop: -40 }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, flexWrap: 'wrap' }}>
            <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(124,58,237,0.2)', border: '4px solid #0D0D14', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 32, color: C.purple2, overflow: 'hidden', flexShrink: 0 }}>
            ...
            <div style={{ paddingBottom: 4, flex: 1 }}>
              <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 24, letterSpacing: '-0.04em', color: C.text, marginBottom: 3 }}>{profile?.display_name || 'Sin nombre'}</h1>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 14, padding: 4 }}>
        {([['perfil', '⚙️ Editar perfil'], ['comunidades', '🌐 Mis comunidades'], ['cursos', '📚 Mi progreso']] as const).map(([t, label]) => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: '9px', borderRadius: 10, background: tab === t ? 'linear-gradient(135deg, #7C3AED, #9F67FF)' : 'transparent', color: tab === t ? '#fff' : C.muted, border: 'none', cursor: 'pointer', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 13 }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div>
          {communities.length > 0 ? communities.map((m: any) => {
            ...
            return (
              <Link key={m.id} href={`/comunidades/${comm?.slug}`} style={{ textDecoration: 'none', display: 'block', marginBottom: 12 }}>
                <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 18, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 15, color: C.text, marginBottom: 3 }}>{comm?.name}</div>
                    <div style={{ fontSize: 12, color: C.muted }}>{comm?.member_count ?? 0} miembros · {m.points ?? 0} pts · {m.role}</div>
                  </div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 20, color: accent }}>{m.points ?? 0} pts</div>
                </div>
              </Link>
            )
          }) : (
            <div style={{ textAlign: 'center', padding: '60px 24px', background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20 }}>
              ...
              <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 18, color: C.text, marginBottom: 8 }}>Sin comunidades</h3>
              <p style={{ fontSize: 14, color: C.muted, marginBottom: 20 }}>Únete a una comunidad para empezar</p>
              <Link href="/comunidades" style={{ display: 'inline-flex', padding: '10px 20px', borderRadius: 12, background: 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', textDecoration: 'none', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14 }}>Explorar comunidades</Link>
            </div>
          )}
        </div>
      )}
      ...
      {tab === 'cursos' && (
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, padding: 24, textAlign: 'center' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>📚</div>
          <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 18, color: C.text, marginBottom: 8 }}>
          ...
          <Link href="/cursos" style={{ display: 'inline-flex', padding: '10px 20px', borderRadius: 12, background: 'linear-gradient(135deg, #7C3AED, #9F67FF)', color: '#fff', textDecoration: 'none', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14 }}>Ver cursos disponibles</Link>
        </div>
      )}
    </div>
  )
