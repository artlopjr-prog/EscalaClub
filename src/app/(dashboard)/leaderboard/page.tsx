  ...
  return (
    <div style={{ padding: 32, maxWidth: 800, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 34, letterSpacing: '-0.04em', color: C.text, marginBottom: 4 }}>Leaderboard 🏆</h1>
      ...
      {myRank >= 0 && (
        <div style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 16, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16 }}>
          <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 28, color: C.purple2, minWidth: 40, textAlign: 'center' }}>#{myRank + 1}</div>
          <div>
          ...
          <div style={{ marginLeft: 'auto', fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 24, color: C.gold }}>{myPoints.toLocaleString()} pts</div>
        </div>
      )}
      ...
      <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 20, overflow: 'hidden' }}>
        ...
        {leaders && leaders.length > 0 ? leaders.map((l: any, i) => {
          ...
          return (
            <div key={`${l.user_id}-${i}`} style={{ display: 'grid', gridTemplateColumns: '50px 1fr 100px 80px', gap: 16, padding: '14px 20px', borderBottom: i < leaders.length - 1 ? `1px solid ${C.border}` : 'none', alignItems: 'center', background: isMe ? 'rgba(124,58,237,0.05)' : 'transparent' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(124,58,237,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 13, color: C.purple2, flexShrink: 0, overflow: 'hidden' }}>
                  {'?'}
                </div>
                <div>
              ...
              <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 15, color: C.gold }}>{(l.points ?? 0).toLocaleString()}</div>
          ...
          <div style={{ padding: '60px 24px', textAlign: 'center' }}>
            ...
            <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 18, color: C.text, marginBottom: 8 }}>Sé el primero</h3>
