  ...
  return (
    <div style={{ padding: '32px 28px', maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 'clamp(26px,5vw,38px)', letterSpacing: '-0.04em', marginBottom: 6 }}>
          🔔 Notificaciones
        </h1>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 16, marginBottom: 3 }}>WhatsApp</div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={sendCode} disabled={loading}
                  style={{ padding: '11px 18px', borderRadius: 10, background: WA_GREEN, border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: 'Syne, sans-serif', cursor: loading ? 'default' : 'pointer', opacity: loading ? .7 : 1, display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input className="input" value={code} onChange={e => setCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
                  placeholder="123456" maxLength={6} style={{ flex: 1, fontSize: 20, letterSpacing: '0.2em', textAlign: 'center', fontFamily: 'Syne, sans-serif', fontWeight: 700 }}
                  onKeyDown={e => e.key === 'Enter' && verifyCode()} />
                <button onClick={verifyCode} disabled={loading || code.length !== 6}
                  style={{ padding: '11px 18px', borderRadius: 10, background: code.length === 6 ? WA_GREEN : 'var(--bg3)', border: 'none', color: code.length === 6 ? '#fff' : 'var(--muted)', fontSize: 13, fontWeight: 700, fontFamily: 'Syne, sans-serif', cursor: code.length === 6 ? 'pointer' : 'default', display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
        ...
        {isConnected && (
          <div>
            ...
            <button onClick={disconnect} style={{ background: 'none', border: '1px solid var(--border2)', borderRadius: 8, padding: '7px 14px', color: 'var(--muted)', fontSize: 12, cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      {recentLogs.length > 0 && (
        <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 20, padding: '20px 24px' }}>
          <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 15, marginBottom: 16 }}>
            Historial reciente
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 20, padding: '20px 24px', marginTop: 20, opacity: 0.6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          ...
          <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 15 }}>Email digest</div>
