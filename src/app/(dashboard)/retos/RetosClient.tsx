  ...
  return (
    <div style={{ padding: '28px 28px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 28, letterSpacing: '-0.04em', color: C.text, marginBottom: 4 }}>
            ⚡ Retos
          </h1>
        ...
        {(['home','detail','leaderboard', ...(canCreate ? ['crear'] : [])] as View[]).map((v, i) => {
          ...
          return (
            <button key={v} onClick={() => v !== 'detail' ? setView(v) : (selectedChallenge && setView(v))}
              style={{ padding: '7px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: active ? C.bg3 : 'transparent', color: active ? C.text : C.muted2, fontFamily: 'Outfit, sans-serif', transition: 'all .15s' }}>
              {labels[v]}
            </button>
          )
        })}
            ...
            <div style={{ background: c.origin === 'platform' ? `linear-gradient(135deg,rgba(240,165,0,0.07),rgba(240,165,0,0.02))` : C.bg1, border: `1px solid ${c.origin === 'platform' ? C.borderGold : C.border}`, borderRadius: 16, padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 20 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <h2 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 20, marginBottom: 4 }}>{c.title}</h2>
              ...
              {p && (
                <div style={{ display: 'flex', gap: 20, alignItems: 'center', marginBottom: 18, background: C.bg2, borderRadius: 12, padding: '12px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <div>
                      <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 30, color: C.gold, lineHeight: 1 }}>{p.current_streak}</div>
              ...
              {c.status === 'active' && (
                <button
                  ...
                  style={{
                    ...
                    fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 14, cursor: checkedToday ? 'default' : 'pointer',
                ...
                {(['global','country','community'] as LBTab[]).map((t, i) => {
                  ...
                  return (
                    <button key={t} onClick={() => loadLeaderboard(c.id, t)} style={{
                      ...
                      color: lbTab === t ? C.text : C.muted2, fontFamily: 'Outfit, sans-serif',
                    }}>{labels[i]}</button>
                  )
                })}
              </div>
                  ...
                  {lbData.slice(0, 10).map((row: any, i) => {
                    ...
                    return (
                      <div key={row.user_id} style={{
                        ...
                        <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 12, width: 22, textAlign: 'center', color: i < 3 ? [C.gold, '#9ba8b5', '#c47c2a'][i] : isMe ? C.purpleL : C.muted }}>
                          {i < 3 ? medals[i] : `#${i + 1}`}
                        </div>
                        ...
                        <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 13, color: i === 0 ? C.gold : isMe ? C.purpleL : C.muted2 }}>{row.current_streak}🔥</div>
                      </div>
                    )
                  })}
                </div>
      ...
      {view === 'leaderboard' && (
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 16, padding: 22 }}>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 18, marginBottom: 4 }}>🏆 Leaderboard Global de Retos</div>
            ...
            {(['global','country'] as LBTab[]).map((t, i) => {
              ...
              return (
                <button key={t} onClick={() => { setLbTab(t); if (platformChallenges[0]) loadLeaderboard(platformChallenges[0].id, t) }}
                  style={{ padding: '6px 14px', borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: lbTab === t ? C.bg3 : 'transparent', color: lbTab === t ? C.text : C.muted2, fontFamily: 'Outfit, sans-serif' }}>
                  {labels[i]}
                </button>
              )
            })}
              ...
              {lbData.slice(0, 20).map((row: any, i) => {
                ...
                return (
                  <div key={row.user_id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: `1px solid ${C.border}`, background: isMe ? `rgba(124,58,237,0.07)` : 'transparent' }}>
                    <div style={{ width: 26, textAlign: 'center', fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 13, color: i < 3 ? [C.gold, '#9ba8b5', '#c47c2a'][i] : isMe ? C.purpleL : C.muted }}>
                      {i < 3 ? medals[i] : `#${i + 1}`}
                    </div>
                    ...
                    <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 14, color: i === 0 ? C.gold : isMe ? C.purpleL : C.muted2 }}>{row.current_streak}🔥</div>
                  </div>
                )
              })}
            </div>
      ...
      {view === 'crear' && canCreate && (
        <div style={{ background: C.bg1, border: `1px solid ${C.border}`, borderRadius: 16, padding: 24 }}>
          <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 19, marginBottom: 3 }}>Crear Nuevo Reto</div>
...
function SectionHeader({ title, badge, badgeColor, badgeText }: { title: string; badge: string; badgeColor: string; badgeText: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, marginTop: 4 }}>
      <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 15 }}>{title}</div>
  ...
  return (
    <div onClick={onOpen} style={{
      ...
      <div style={{ padding: '13px 14px' }}>
        ...
        <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 14, marginBottom: 5, lineHeight: 1.2 }}>{c.title}</div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} onClick={e => e.stopPropagation()}>
          ...
          {c.status === 'active' && (
            p ? (
              ...
              <button onClick={onJoin} style={{ padding: '5px 12px', borderRadius: 8, border: 'none', fontSize: 11, fontWeight: 700, background: 'rgba(124,58,237,0.2)', color: '#9F67FF', cursor: 'pointer', fontFamily: 'Outfit, sans-serif' }}>
                + Unirme
              </button>
            )
          )}
...
function EmptyState({ icon, title, sub }: { icon: React.ReactNode; title: string; sub: string }) {
  return (
    <div style={{ textAlign: 'center', padding: '40px 20px', background: '#0D0D14', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', marginBottom: 24 }}>
      ...
      <div style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 15, marginBottom: 6 }}>{title}</div>
