  ...
  return (
    <div style={{ padding: '32px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 34, letterSpacing: '-0.04em', marginBottom: 4 }}>Cursos 📚</h1>
      ...
      {courses && courses.length > 0 ? (
              ...
              <Link key={course.id} href={`/cursos/${course.id}`} style={{ textDecoration: 'none', display: 'block' }}>
                <div style={{ background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 20, overflow: 'hidden', transition: 'all 0.2s', cursor: 'pointer' }}>
                  ...
                  <div style={{ padding: '16px' }}>
                    ...
                    <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 14, letterSpacing: '-0.03em', marginBottom: 12, lineHeight: 1.3 }}>{course.title}</h3>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 11, color: 'var(--muted)' }}>
                        {new Date(course.created_at).toLocaleDateString('es', { day: 'numeric', month: 'short' })}
                      </span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, fontFamily: 'Syne, sans-serif', fontWeight: 700, color: 'var(--purple2)' }}>
        ...
        <div style={{ textAlign: 'center', padding: '80px 24px', background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: 20 }}>
          ...
          <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 20, marginBottom: 8 }}>Sin cursos disponibles</h3>
